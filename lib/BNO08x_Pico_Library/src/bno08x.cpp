...
bool BNO08x::_init(int32_t sensor_id) {
  ...
  printf("sh2_getProdIds status=%d\n", status);
  if (status != SH2_OK) return false;
...
static int i2chal_read(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len,
  ...
  // Polling corto (2ms): sin INT, esto evita que sh2_open/sh2_getProdIds fallen
  absolute_time_t t0 = get_absolute_time();
  while (true) {
    if (i2c_read(header, 4)) {
      break; // ya hay header válido
    }

    // si pasaron 2000us (2ms) y no hubo data, no es "error fatal": solo "no hay nada aún"
    if (absolute_time_diff_us(t0, get_absolute_time()) > 2000) {
      return 0;
    }
